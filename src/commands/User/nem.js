...
module.exports = {
  ...
  async executeMessage(message, args) {
    ...
    // Gây sát thương
    // Bánh không có damage cứng -> Tính dựa trên giá (50 xu -> 10 dmg, 350 xu -> 70 dmg)
    const damage = cakeInfo.damage || Math.max(10, Math.floor(cakeInfo.basePrice / 5));
    victim.hp -= damage;
  ...
  async handleComponent(interaction) {
    ...
    if (action === 'open') {
      ...
      const options = BAKED_KEYS.filter(k => (user.inventory[k] || 0) > 0).map(k => {
        ...
        const damage = info.damage || Math.max(10, Math.floor(info.basePrice / 5));
        return { label: `${info.emoji} ${info.name}`, description: `Sát thương: ${damage} HP (Có: ${user.inventory[k]})`, value: k };
      });
    ...
    if (action === 'throw') {
      ...
      if (targetId === interaction.user.id) return interaction.editReply({ embeds: [errorEmbed('Không thể tự ném chính mình!')], components: [row(btn(`nem:open:${targetId}`, '◀ Quay Lại', 'Secondary'))] });
      if (targetUser.bot) return interaction.editReply({ embeds: [errorEmbed('Bot không có cảm giác đâu nha!')], components: [row(btn(`nem:open:${targetId}`, '◀ Quay Lại', 'Secondary'))] });
      ...
      if (!attacker.inventory[itemKey] || attacker.inventory[itemKey] < 1) {
        return interaction.editReply({ embeds: [errorEmbed('Bạn hết loại bánh này rồi!')], components: [row(btn(`nem:open:${targetId}`, '◀ Quay Lại', 'Secondary'))] });
      }
      ...
      await Promise.all([attacker.save(), victim.save()]);
      return interaction.editReply({ embeds: [bakeryEmbed('🎯 Đại Chiến Bánh Ngọt', resultMsg, embedColor)], components: [row(btn(`nem:open:${targetId}`, '🎯 Tiếp Tục Ném', 'Primary'), btn('menu:section:social', '◀ Về Xã Hội', 'Secondary'))] });
    }
  }
};