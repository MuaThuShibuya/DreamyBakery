...
const { bakeryEmbed, errorEmbed, successEmbed } = require('../utils/embeds');
  ...
  async executeMessage(message, args) {
    ...
    if (sub === 'tra' || sub === 'pay') {
      ...
      return message.reply({ embeds: [successEmbed(
        ...
        [
          ...
          `💰 **Số dư ví:** ${user.coins.toLocaleString('vi-VN')} xu`
        ].join('\n')
      )]});
    }
    ...
    if (message.mentions.users.size > 0) {
      if (!message.member.permissions.has(PermissionFlagsBits.Administrator)) {
        return message.reply({ embeds: [errorEmbed('Chỉ Admin server mới có quyền duyệt khoản vay!')] });
      }
      ...
      if (isNaN(interest) || interest < 0) {
        return message.reply({ embeds: [errorEmbed('Lãi suất không hợp lệ (phải từ 0 trở lên)!')] });
      }
      ...
      return message.reply({ embeds: [successEmbed(
        ...
        [
          ...
          '',
          `💳 Ví hiện tại của người vay: **${user.coins.toLocaleString('vi-VN')} xu**`,
